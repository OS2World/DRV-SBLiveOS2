/* $Id: waverec.cpp,v 1.1 2000/04/23 14:55:22 ktk Exp $ */
...
/******************************************************************************/
/* WaveRec_Irq_Handler                                                        */
/* This is the ISR for the WAVEREC object. All ISRs must be Global in nature. */
/* This routine will be called by the IRQ object when it receives an interrupt*/
/* From the OS/2 kernal and the WAVEREC object has previously registered this */
...
/*                                                                            */
/******************************************************************************/
BOOL __far __loadds __saveregs WaveRec_Irq_Handler(void)
{
   PWAVESTREAM pstream;

      // see if there is an active stream
      pstream = (PWAVESTREAM)FindActiveStream(STREAM_WAVE_CAPTURE);
...
/*                                                                            */
/******************************************************************************/
virtual int WAVEREC::Start(STREAM *stream)
{
...
/*                                                                            */
/******************************************************************************/
virtual int WAVEREC::Stop(STREAM *stream)
{
   return OSS16_StopStream(stream) != TRUE;
}

virtual void WAVEREC::ConfigDev(STREAM *stream, PWAVECONFIGINFO pConfigInfo)
{
...
/******************************************************************************/
/* WAVEREC::WAVEREC(USHORT PrimeDma, USHORT SecDma, USHORT Irq)               */
...
WAVEREC::WAVEREC():
   WAVEAUDIO(AUDIOHW_WAVE_CAPTURE)
{
    _vSetup();
